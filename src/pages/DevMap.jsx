...
export default function DevMap() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const lb = await loadLeaderboardData();
        setData(lb);
      } catch (e) {
    ...
    return (
      <main className="min-h-screen relative overflow-hidden">
        ...
        <div className="flex items-center justify-center h-96 flex-col gap-3">
          <span className="font-mono text-sm text-error uppercase tracking-widest">{error}</span>
          <button
            type="button"
            onClick={() => window.location.reload()}
            className="font-mono text-xs border border-outline-variant px-4 py-2 hover:text-primary"
          >
            RETRY
          </button>
        </div>
