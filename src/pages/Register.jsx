...
export default function Register({ onChangeTab }) {
        ...
        if (stored) {
          ...
          if (Array.isArray(parsed)) {
            localRegs = parsed.map((dev) => ({
              ...
              timeAgo: dev.registeredAt ? formatTimeAgo(dev.registeredAt) : dev.timeAgo || 'Unknown'
            }));
          }
        }
      } catch {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const alreadyRegistered = stored.some(
        (entry) => entry.username?.toLowerCase() === trimmedUsername.toLowerCase()
      );

      if (alreadyRegistered) {
        setErrorMsg('SYSTEM_INFO: Already Registered.');
        setStatus('error');
        return;
      }
    } catch {
      // Continue with registration if localStorage cannot be read.
    }
    ...
    try {
      ...
      const profileFailed =
        !isValidLocation || !hasEnoughRepos || !hasEnoughFollowers || !isOldEnough;
      if (profileFailed) {
        const reasons = [];
        if (!isValidLocation)
          reasons.push(`Location '${data.location || 'not set'}' must include 'Pakistan'`);
        ...
        if (!isOldEnough)
          reasons.push(`Account is ${ageDays}d old (need >=${CRITERIA.MIN_ACCOUNT_AGE_DAYS}d)`);
      ...
      try {
        for (let page = 1; page <= 2; page++) {
          const evRes = await fetch(
            `https://api.github.com/users/${data.login}/events?per_page=100&page=${page}`
          );
          if (stale()) return;
          if (!evRes.ok) break;
      ...
      try {
        for (let page = 1; ; page++) {
          const repoRes = await fetch(
            `https://api.github.com/users/${data.login}/repos?per_page=100&page=${page}`
          );
          if (stale()) return;
          if (!repoRes.ok) break;
      ...
      if (!activityPassed) {
        ...
        if (!hasContributions)
          reasons.push(
            `${count} meaningful contributions in 60d (need >=${CRITERIA.MIN_CONTRIBUTIONS_60D})`
          );
        if (!hasNoLongGaps)
          reasons.push(
            `Longest inactivity gap: ${longestGap}d (max ${CRITERIA.MAX_INACTIVITY_GAP_DAYS}d)`
          );
        setGatesPassed(false);
        setStatus('error');
      ...
      try {
        ...
        setRecentDevs((prev) => {
          if (prev.find((p) => p.username.toLowerCase() === newReg.username.toLowerCase())) {
            return prev;
          }
          ...
          return [
            {
              ...newReg,
              timeAgo: formatTimeAgo(newReg.registeredAt)
            },
            ...prev
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 sm:px-6 sm:py-8 md:py-12 relative z-10">
        <div className="mb-8 border-l-4 border-primary pl-4 sm:mb-12 sm:pl-6">
          ...
          <p className="font-mono text-xs sm:text-sm text-outline max-w-xl uppercase tracking-widest leading-relaxed">
            Entry point for the high-performance engineering cluster. Provide your GitHub identifier
            to synchronize.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 border border-outline-variant">
          <div className="lg:col-span-5 p-4 sm:p-6 lg:p-8 bg-surface-container-lowest border-b lg:border-b-0 lg:border-r border-outline-variant flex flex-col justify-between">
            <div className="space-y-6 sm:space-y-8">
              <div>
                ...
                <p className="mt-4 font-mono text-[10px] text-outline leading-relaxed uppercase">
                  By proceeding, you authorize Rankistan to fetch your public contributions and
                  profile metadata.
                </p>
              </div>
              ...
              <div className="pt-6 sm:pt-8 border-t border-outline-variant/30">
                <h3 className="font-mono text-xs text-outline-variant uppercase mb-4">
                ...
                <ul className="space-y-3">
                  ...
                  <li className="flex items-start gap-3 text-xs font-mono text-outline">
                    {renderCheckIcon(checks.contributions)}{' '}
                    <span>30 meaningful contributions in last 60 days</span>
                  </li>
                  <li className="flex items-start gap-3 text-xs font-mono text-outline">
                    {renderCheckIcon(checks.noLongGaps)}{' '}
                    <span>No inactivity gap longer than 30 days</span>
                  </li>
                </ul>
                <p className="mt-4 font-mono text-[9px] text-outline/60 uppercase leading-relaxed">
                  Meaningful contributions: pushes, pull requests, issues, and releases. All checks
                  are verified in real-time against the GitHub API.
                </p>
              </div>
            </div>
            ...
            {status === 'complete' && (
              <div
                ref={statusPanelRef}
                className="mt-6 sm:mt-8 font-mono text-[11px] sm:text-xs text-tertiary border border-tertiary/30 bg-tertiary/10 p-3 sm:p-4 animate-pulse leading-relaxed"
              >
                &gt; Profile Validated.
                <br />
                &gt; All criteria passed.
                <br />
                &gt; Estimated Score: {profileData?._calculatedScore}{' '}
                {profileData?._agePenaltyApplied && (
                  <span className="text-error">(0.5x New Account Penalty Applied)</span>
                )}
                <br />
                <br />
                <span className="text-tertiary/80">
                  * Note: The leaderboard updates periodically in batches. Your profile and official
                  score will be visible after the next global synchronization.
                </span>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-7 bg-surface p-0 flex flex-col min-h-0">
            ...
            >
              ...
              {recentDevs.map((dev, idx) => (
                <div
                  key={dev.username + idx}
                  className={`flex items-center gap-3 border-b p-3 sm:grid sm:grid-cols-12 sm:items-center sm:gap-0 sm:p-0 ${dev.isNew ? 'border-tertiary bg-tertiary/5' : 'border-outline-variant/50 hover:bg-surface-container-low'} transition-colors group cursor-default relative overflow-hidden`}
                >
                  {dev.isNew && (
                  ...
                  <div className="shrink-0 sm:col-span-2 sm:p-4 sm:flex sm:justify-center relative z-10">
                    <div
                      className={`w-10 h-10 border ${dev.isNew ? 'border-tertiary shadow-[0_0_10px_rgba(80,184,94,0.4)]' : 'border-outline-variant grayscale group-hover:grayscale-0 transition-all'} overflow-hidden`}
                    >
                      <img
                        alt={`${dev.username} profile`}
                  ...
                  <div className="min-w-0 flex-1 sm:col-span-6 sm:p-4 relative z-10">
                    <div
                      className={`font-headline font-bold text-sm tracking-tight uppercase truncate ${dev.isNew ? 'text-tertiary' : 'text-on-surface'}`}
                    >
                      {dev.username}
                    </div>
                    <div className="font-mono text-[10px] text-outline uppercase tracking-tighter line-clamp-1">
                      {dev.role} &bull; {dev.location}
                    </div>
                  </div>
                  <div className="shrink-0 text-right sm:col-span-4 sm:p-4 relative z-10">
                    <div
                      className={`inline-block px-2 py-1 font-mono text-[9px] uppercase ${dev.isNew ? 'bg-tertiary-container text-on-tertiary-container' : 'bg-secondary-container text-on-secondary-container'}`}
                    >
                      {typeof dev.score !== 'undefined' ? `Score: ${dev.score}` : dev.stack}
                    </div>
                    <div
                      className={`font-mono text-[9px] mt-1 uppercase ${dev.isNew ? 'text-tertiary' : 'text-outline-variant'}`}
                    >
                      {dev.timeAgo}
                    </div>
                  </div>
            ...
            <div className="p-3 sm:p-4 bg-surface-container-highest border-t border-outline-variant text-center">
              <button
                ...
                className="font-mono text-[10px] text-primary uppercase tracking-widest hover:underline min-h-11 sm:min-h-0 inline-flex items-center justify-center"
              >
        ...
        <div className="mt-8 sm:mt-12 grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          <div className="p-4 sm:p-6 border border-outline-variant bg-surface-container-lowest">
            ...
            <h4 className="font-headline font-bold uppercase text-on-surface mb-2">
              Global Routing
            </h4>
            <p className="font-body text-xs text-outline leading-relaxed">
              Your profile is indexed across our global edge network, making your skills
              discoverable by international tech recruiters.
            </p>
          </div>
          <div className="p-4 sm:p-6 border border-outline-variant bg-surface-container-lowest">
            ...
            <p className="font-body text-xs text-outline leading-relaxed">
              Track your ranking within the Pakistani developer ecosystem based on commit velocity,
              impact, and code quality.
            </p>
          </div>
          <div className="p-4 sm:p-6 border border-outline-variant bg-surface-container-lowest">
            ...
            <p className="font-body text-xs text-outline leading-relaxed">
              Built for engineers, by engineers. Your data belongs to you, exported anytime via our
              public GraphQL API.
            </p>
          </div>
        </div>
